...
public final class AutoPhotoLayout extends LinearLayoutCompat {
    ...
    private BaseAppFragment mDelegate = null;
    /**
     * 一行几张图
     */
    ...
    /**
     * 存储所有的View；存储方式是一行一行存储的；如果有两行就存储两行所有的View；
     */
    ...
    public AutoPhotoLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //从定义的attrs.xml中将值取出，获取在布局中传入的属性值
    ...
    public final void setDelegate(BaseAppFragment delegate) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < cCount; i++) {
            ...
            //换行
            if (childWidth + lineWith > sizeWith - getPaddingLeft() - getPaddingRight()) {
                //对比得到最大宽度
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < cCount; i++) {
            ...
            if (childWith + lineWidth + lp.leftMargin + lp.rightMargin >
                ...
                //记录当前一行的Views，
                //此处不能直接add(mLineViews),ArrayList不能深拷贝，后面的clear()会改变原值
